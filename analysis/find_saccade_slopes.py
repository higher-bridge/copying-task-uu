...
def fit_movement_regression(results, condition, X='Distance (pixels)', Y='Duration (ms)',
        ...
        for trial in list(fix_df_c['Trial'].unique()):
            if trial not in constants.EXCLUDE_TRIALS:
                ...
                for i in range(len(saccades)):
                    ...
                    angle_change = compute_angle_change((df[features[0]], df[features[1]]),
                                                        (df[features[2]], df[features[3]]))
...
if __name__ == '__main__':
    ...
    for condition in sorted(list(results['Condition'].unique())):
        ...
        intercept, coef, r_squared, p = fit_movement_regression(results, condition, X='Distance (pixels)', x_limit=1800)
    ...
    for condition in sorted(list(results['Condition'].unique())):
        # plot_saccades(results, condition)
        intercept, coef, r_squared, p = fit_movement_regression(results, condition, X='Distance (fitts)',
